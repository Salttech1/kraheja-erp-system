...
public class OutrateServiceImpl implements OutrateService {
	...
	@Override
	public ResponseEntity<?> fetchOutrateByBldgcodeAndFlatnumAndWingAndStartdate(String  bldgcode, String  flatnum, String  wing, String  startdate) {
		Outrate outrateEntity = this.outrateRepository.findByOutrateCK_OutrBldgcodeAndOutrateCK_OutrFlatnumAndOutrateCK_OutrWingAndOutrateCK_OutrStartdate(bldgcode, flatnum, wing, startdate);
	...
	//		Outrate outrateEntity = this.outrateRepository.findByOutrateCK_OutrBldgcodeAndOutrateCK_OutrFlatnumAndOutrateCK_OutrWing(bldgcode, flatnum, wing);
	//		logger.info("OutrateEntity :: {}", outrateEntity);
	//		if (outrateEntity != null) {
	//			return ResponseEntity.ok(ServiceResponseBean.builder()
	//									.data(OutrateEntityPojoMapper.fetchOutrateEntityPojoMapper
	...
	@Override
	public ResponseEntity<?> fetchOutrateByBldgcodeAndFlatnumAndWing(String  bldgcode, String  flatnum, String  wing) {
		List<Outrate> outrateEntity = this.outrateRepository.findByOutrateCK_OutrBldgcodeAndOutrateCK_OutrFlatnumAndOutrateCK_OutrWing(bldgcode, flatnum, wing);
		logger.info("OutrateEntity :: {}", outrateEntity);
		if (outrateEntity != null) {
			return ResponseEntity.ok(ServiceResponseBean.builder()
					.data(OutrateEntityPojoMapper.fetchOutrateEntityPojoMapper
		...
		if(Objects.nonNull(outrateRequestBeanList)) {
			...
			outrateRequestBeanList.stream().map(outrateRequestBean->{
				...
				if(outrateRequestBean.getIsDelete()) {
					deleteOutrate(outrateRequestBean.getBldgcode().trim() , outrateRequestBean.getFlatnum().trim() , outrateRequestBean.getWing().trim(),outrateRequestBean.getStartdate().trim()) ;
					}
				else
					{
						Outrate outrateEntity = this.outrateRepository.findByOutrateCK_OutrBldgcodeAndOutrateCK_OutrFlatnumAndOutrateCK_OutrWingAndOutrateCK_OutrStartdate(outrateRequestBean.getBldgcode() , outrateRequestBean.getFlatnum() , outrateRequestBean.getWing(),outrateRequestBean.getStartdate());

						if(Objects.nonNull(outrateEntity )) {
							outrateEntityList.add(OutrateEntityPojoMapper.updateOutrateEntityPojoMapper.apply(outrateEntity, outrateRequestBean)) ;
						}
					else
						{
							outrateEntityList.addAll(OutrateEntityPojoMapper.addOutrateEntityPojoMapper.apply(Arrays.asList(outrateRequestBean))) ;
							
						}
				}
				
				auditTranser.setValue(outrateRequestBean.getBldgcode().trim() + outrateRequestBean.getFlatnum().trim() + outrateRequestBean.getWing().trim() ) ; 
//
	...
	public ResponseEntity<?> deleteOutrate(String bldgcode , String flatnum , String wing , String  startdate) {
		...
		if(Objects.nonNull(outrateEntity)) {
			...
			//					GenericAuditContextHolder.getContext().setTransactionNo(outrateEntity.getOutrateCK().getOutrBldgcode() + outrateEntity.getOutrateCK().getOutrFlatnum() + outrateEntity.getOutrateCK().getOutrWing() + outrateEntity.getOutrateCK().getOutrStartdate());
			//					GenericAuditContextHolder.getContext().setAuditable(Boolean.TRUE);
