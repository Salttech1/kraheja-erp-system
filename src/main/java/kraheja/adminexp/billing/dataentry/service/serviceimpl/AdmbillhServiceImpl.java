...
public class AdmbillhServiceImpl implements AdmbillhService {
	...
	public ResponseEntity<?> fetchAdmbillhBySer(String  ser) {
		...
		if (admbillhEntity != null) {
			Party partyEntity = this.partyRepository.findByPartyCk_ParPartycodeAndPartyCk_ParClosedateAndPartyCk_ParPartytype(admbillhEntity.getAdblhPartycode().trim(),
					CommonUtils.INSTANCE.closeDateInLocalDateTime(),admbillhEntity.getAdblhPartytype().trim());
			
			Address addressEntity = this.addressRepository.findByAddressCK_AdrAdownerAndAddressCK_AdrAdsegmentAndAddressCK_AdrAdtypeAndAddressCK_AdrAdser(admbillhEntity.getAdblhPartycode().trim(), CommonConstraints.INSTANCE.adrAdsegment, CommonConstraints.INSTANCE.addresstype, CommonConstraints.INSTANCE.adrAdser);
			logger.info("Address :: {}", addressEntity);
			
			List <Admbilld> admbilldEntityList = this.admbilldRepository.findByAdmbilldCK_AdbldSer(ser);
			logger.info("AdmbillhEntity :: {}", admbillhEntity);
			
			Admadvance admadvanceEntityList = this.admadvanceRepository.findByAdmadvanceCK_AdvnSer(ser);
			logger.info("AdmbillhEntity :: {}", admbillhEntity);
			
			return ResponseEntity.ok(ServiceResponseBean.builder()
									.data(AdmbillhEntityPojoMapper.fetchAdmbillhEntityPojoMapper
									.apply(new Object[] { Arrays.asList(admbillhEntity), partyEntity , addressEntity, admbilldEntityList,admadvanceEntityList })
									).status(Boolean.TRUE).build());
			}
		return ResponseEntity.ok(ServiceResponseBean.builder().status(Boolean.FALSE).message("No record found for your selections in Admbillh").build());
	}
	...
	public ResponseEntity<?> addAdmbillh(AdmbillhRequestBean admbillhRequestBean) throws ParseException {
		...
		String ser = GenericCounterIncrementLogicUtil.generateTranNoWithSite("#NSER","BOSER", GenericAuditContextHolder.getContext().getSite());
		...
		if(Objects.nonNull(admbillhRequestBean)) {
			if(CollectionUtils.isNotEmpty(admbillhRequestBean.getAdmbilldRequestBeanList())) {
				this.admbillhRepository.saveAll(AdmbillhEntityPojoMapper.addAdmbillhEntityPojoMapper.apply(new Object[] {Arrays.asList(admbillhRequestBean), ser}));
			}
			if(CollectionUtils.isNotEmpty(admbillhRequestBean.getAdmbilldRequestBeanList())) {
				this.admbilldRepository.saveAll(AdmbilldEntityPojoMapper.addAdmbilldEntityPojoMapper.apply(new Object[] {admbillhRequestBean.getAdmbilldRequestBeanList()}));
			}
			
			if(CollectionUtils.isNotEmpty(admbillhRequestBean.getAdmadvanceRequestBeanList())) {
				this.admadvanceRepository.saveAll(AdmadvanceEntityPojoMapper.addAdmadvancePojoEntityMapper.apply(new Object[] {admbillhRequestBean.getAdmadvanceRequestBeanList()}));
			}
		}
		...
		this.addressRepository.save(AddressMapper.addAddressPojoEntityMapping.apply(new Object[] {admbillhRequestBean.getAddressRequestBean(), ser}));
		...
		this.partyRepository.save(PartyMapper.addPartyPojoEntityMapping.apply(new Object[] {admbillhRequestBean.getPartyRequestBean(), GenericAuditContextHolder.getContext().getSite(), admbillhRequestBean.getPartycode()}));
...
//}
	...
	public ResponseEntity<?> updateAdmbillh(AdmbillhRequestBean admbillhRequestBean) throws ParseException {
		...
		List<Admbilld> admbilldEntityList = this.admbilldRepository.findByAdmbilldCK_AdbldSer(admbillhRequestBean.getSer()) ; 
		...
		if(Objects.nonNull(admbillhEntity)) {
			...
			if(Objects.nonNull(admbillhRequestBean))
				this.admbillhRepository.save(AdmbillhEntityPojoMapper.updateAdmbillhEntityPojoMapper.apply(admbillhEntity, admbillhRequestBean));
			...
			if(CollectionUtils.isNotEmpty(admbillhRequestBean.getAdmbilldRequestBeanList())) {
				...
				if (CollectionUtils.isNotEmpty(admbilldRequestBeanToAdd))
					this.admbilldRepository.saveAll(AdmbilldEntityPojoMapper.addAdmbilldEntityPojoMapper.apply(new Object[] {admbilldRequestBeanToAdd,admbillhEntity.getAdmbillhCK().getAdblhSer()}));
				
				admbilldEntityList.stream().map(admbilldEntity -> {
					admbillhRequestBean.getAdmbilldRequestBeanList().stream().filter(filter ->{
						return admbilldEntity.getAdmbilldCK().getAdbldSer().trim().equals(filter.getSer().trim()) && admbilldEntity.getAdmbilldCK().getAdbldLineno().equals(filter.getLineno()) && !filter.getIsAdd();
					}).map(admbilldRequestBean ->{
						this.admbilldRepository.save(AdmbilldEntityPojoMapper.updateAdmbilldEntityPojoMapper.apply(admbilldEntity, admbilldRequestBean));
			...
			if(Objects.isNull(addressEntity)) {
				...
				admbillhRequestBean.getAddressRequestBean().setTopser(CommonConstraints.INSTANCE.adrAdser);
				admbillhRequestBean.getAddressRequestBean().setState(admbillhRequestBean.state);
				this.addressRepository.save(AddressMapper.addAddressPojoEntityMapping.apply(new Object[] {admbillhRequestBean.getAddressRequestBean(), SiteFromDBEntity, admbillhRequestBean.getSer().trim()}));
			}
			else {
				this.addressRepository.save(AddressMapper.updateAddressPojoEntityMapping.apply(addressEntity, admbillhRequestBean.getAddressRequestBean()));
			}
				...
				if(Objects.nonNull(admbillhRequestBean.getPartyRequestBean())) {
					admbillhRequestBean.getPartyRequestBean().setGstno(admbillhRequestBean.gstNumber);
					this.partyRepository.save(PartyMapper.updatePartyEntityPojoMapper.apply(partyEntity, admbillhRequestBean.getPartyRequestBean()));
				}
	...
	public ResponseEntity<?> checkserExists(String ser) {
		...
		logger.info("AdmbillhEntity :: {}", admbillhEntity);
		if (admbillhEntity != null) {
			return ResponseEntity.ok(ServiceResponseBean.builder().message(ser +" is exists under Ser " + admbillhEntity.getAdmbillhCK().getAdblhSer()).status(Boolean.TRUE).build());
		}
